...
export function DemoSection({ isDesktop }: DemoSectionProps) {
  ...
  const [busy, setBusy] = React.useState<
    'loading' | 'copy' | 'another' | null
  >('loading');
  const [copyFlash, setCopyFlash] = React.useState(false);
  const [mounted, setMounted] = React.useState(false);
  const busyRef = React.useRef(false);
  const copyFlashTimeoutRef = React.useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  return (
    <View
      ...
      <View
        ...
        style={{
          boxShadow: '0 24px 48px rgba(0,0,0,0.06), 0 4px 12px rgba(0,0,0,0.03)',
          opacity: mounted ? 1 : 0,
